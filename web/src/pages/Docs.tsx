...
/**
 * Operator-facing docs mounted at `/docs`.
 *
 * Ported from `app/docs/page.tsx` on `main`. Adaptations: `next/link`
 * → `react-router-dom` `<Link>`, `Metadata` export dropped (the SPA
 * sets `<title>` once in `index.html`), and Tailwind v4 utility
 * names (`bg-linear-to-*`) rewritten for v3 (`bg-gradient-to-*`).
 */
...
function H2({ id, children }: { id: string; children: React.ReactNode }) {
  return (
    <h2 id={id} className="text-xl sm:text-2xl font-semibold tracking-tight scroll-mt-20 mb-3">
      {children}
    </h2>
  );
}
...
function H3({ children }: { children: React.ReactNode }) {
  return (
    <h3 className="text-sm font-semibold tracking-tight mt-6 mb-2 text-foreground">{children}</h3>
  );
}
...
function DocsHero() {
  return (
    <section className="relative overflow-hidden border-b border-border">
      <div
        aria-hidden="true"
        className="absolute inset-0 -z-10 bg-[radial-gradient(ellipse_at_top,var(--primary)_0%,transparent_60%)] opacity-15"
      />
  ...
  return (
    <section>
      ...
      <P>
        ...
        <Code>Ctrl/Cmd + N</Code>), describe the work in plain prose, and submit. The coordinator
        picks the right repo(s), spawns child agents, streams their output live, runs the verify
        chain, and aggregates a report when they finish.
      </P>
    ...
    <section>
      ...
      <Pre>{`               ┌──────────────────────────┐
        ...
        └────────────────────┬────────────────────┘
                  ...
                  └──────────────────┘`}</Pre>
      <P>
        Sibling paths are resolved as <Code>../&lt;folder-name&gt;</Code>. Rename or move freely,
        just keep the bridge as a sibling of your app folders.
      </P>
    </section>
  );
}
...
function Configuration() {
  const envVars: { name: string; def: string; purpose: string }[] = [
    ...
    { name: "BRIDGE_ALLOW_BYPASS", def: "—", purpose: "Single-user opt-in for composer bypass mode" },
  ];
  return (
    <section>
      ...
      <ul className="text-sm text-muted-foreground leading-relaxed mb-3 ml-5 list-disc space-y-1">
        <li>
          ...
          <Code>auto-create</Code>), worktree mode, <Code>autoCommit</Code>,{" "}
          <Code>autoPush</Code>
        </li>
        <li>
  ...
  return (
    <section>
      ...
      <P>
        ...
        from the operator&apos;s chat — useful when the bridge is exposed beyond{" "}
        <Code>localhost</Code>.
      </P>
...
function Permissions() {
  return (
    <section>
      ...
      <P>
        By default, agents run in <Code>default</Code> mode for user-typed messages — every tool
        call (<Code>Bash</Code>, <Code>Edit</Code>, <Code>Write</Code>, <Code>Delete</Code>, …)
        pauses behind an inline Allow / Deny popup. Coordinator and auto-spawned children run in
        bypass mode (otherwise they&apos;d hang on the first tool call).
      </P>
      <P>
        ...
        <Code>BRIDGE_ALLOW_BYPASS=1</Code>; the env gate is mirrored on the server so a deploy
        that toggles it off rejects spoofed bypass requests.
      </P>
    </section>
  );
}
...
function Telegram() {
  return (
    <section>
      ...
      <P>
        Pair the operator&apos;s own Telegram account as a private DM channel. Requires a
        numeric user id to dispatch — an <Code>@username</Code> alone is refused so a random DM
  ...
  return (
    <section>
      ...
      <P>
        ...
        webhook testing, or sharing a dev preview from the bridge itself. Two providers ship out
        of the box:
      </P>
      <ul className="text-sm text-muted-foreground leading-relaxed mb-3 ml-5 list-disc space-y-1">
        <li>
          <strong className="text-foreground">localtunnel</strong> — runs via{" "}
          <Code>bunx localtunnel</Code>. Free, no signup, slightly slower, shows an interstitial
          password page on first visit per IP. Custom subdomains are honoured. URL host:{" "}
          <Code>*.loca.lt</Code>.
        </li>
        <li>
          <strong className="text-foreground">ngrok</strong> — faster, no interstitial, needs a
          free authtoken from <Code>dashboard.ngrok.com/get-started/your-authtoken</Code>. The
          bridge one-click installs ngrok via <Code>winget</Code> (Windows), <Code>brew</Code>{" "}
          (macOS), or the official tarball (Linux / mac without brew), and persists the
          authtoken in <Code>~/.claude/bridge.json</Code> with mode <Code>0600</Code>. URL host:{" "}
          <Code>*.ngrok-free.app</Code>.
        </li>
      </ul>
      <P>
        Tunnels are in-memory only — every entry dies when the bridge process exits, and
        restarting the bridge clears the list. Up to 8 concurrent tunnels per bridge instance;
        the page surfaces install state, authtoken state, and live stdout for each running
        tunnel.
      </P>
      <P>
        Anyone with the URL can reach the port — don&apos;t expose services without auth in
        front. Demo-mode deployments (<Code>BRIDGE_DEMO_MODE=1</Code>) redirect{" "}
...
function Scripts() {
  const scripts: { name: string; purpose: string }[] = [
    { name: "dev", purpose: "Start the SPA + Go bridge in dev mode (with .env loaded)" },
    { name: "build", purpose: "Production build of the SPA bundle" },
    { name: "start", purpose: "Run the production Go bridge on port 7777" },
    { name: "serve", purpose: "build then start in one command" },
    { name: "test", purpose: "Run the test suite (Vitest for SPA, go test for the bridge)" },
    { name: "test:watch", purpose: "Vitest in watch mode" },
    { name: "lint", purpose: "Run ESLint + tsc --noEmit on the SPA" },
  ...
  return (
    <section>
      ...
      <P>
        Run any of them with your preferred runtime: <Code>bun run &lt;script&gt;</Code> /{" "}
        <Code>npm run &lt;script&gt;</Code> / <Code>pnpm &lt;script&gt;</Code>.
      </P>
    </section>
  );
}
...
function Faq() {
  const items: { q: string; a: React.ReactNode }[] = [
    ...
    },
    {
      ...
      a: (
        <>
          Only what Claude Code itself sends — the bridge is a local Go server on{" "}
    ...
    },
    {
      ...
      a: (
        <>
          ...
          <Code>/tail</Code>), kill runs, read reports, and approve cross-device logins from any
          chat — with a numeric chat-id allowlist so random DMs are ignored.
        </>
      ),
    },
    {
      ...
      a: (
        <>
          ...
          every coordinator spawn (with task / session variables substituted). The longer static
          reference (recipe table, error codes, finalization procedure, hard rules) is in{" "}
          <Code>prompts/coordinator-playbook.md</Code>, which the coordinator reads on demand.
          Edit either to tune behaviour.
        </>
      ),
    },
  ];
...
function Cta() {
  return (
    <section className="rounded-xl border border-border bg-gradient-to-br from-primary/10 via-card to-card p-6 sm:p-8 mt-12">
