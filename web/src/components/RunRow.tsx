  ...
  return (
    <li className="border-b border-border last:border-b-0">
      ...
      >
        ...
        <span className="font-mono text-micro text-muted-2 truncate max-w-[180px]">
          {run.sessionId.slice(0, 8)}
        </span>
      </button>

      {open && (
        <div className="grid grid-cols-[120px_minmax(0,1fr)] gap-x-6 gap-y-1 border-t border-border bg-surface-2 px-6 py-4 text-small">
          <span className="font-mono text-micro uppercase tracking-wideish text-muted">
            session
          </span>
          <span className="font-mono text-small break-all text-fg">
            {run.sessionId}
          </span>
          {run.parentSessionId && (
            <>
              ...
              <span className="font-mono text-small break-all text-muted">
                {run.parentSessionId}
              </span>
            </>
          )}
