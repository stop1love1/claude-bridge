...
//
// Adapted from the Next-app `_components/NewSessionDialog.tsx`. The
// `useImperativeHandle`-style `openRef` contract is preserved so a
// keyboard shortcut (or anything else) can fire the create.
//
// TODO: a `useCreateSession` mutation hook does not yet exist in
// `web/src/api/queries.ts`. Until the Go bridge ships POST
// /api/sessions, this component takes an `onCreate` callback the
// caller wires up (e.g. directly to a fetch, or a router push). When
// the hook lands, swap the prop pattern for a built-in mutation here.
...
export interface NewSessionDialogProps {
  ...
  /** Programmatic trigger handle. Assigning `.current()` opens/creates. */
...
export function NewSessionDialog({
  ...
  const groups = useMemo(() => {
    ...
    // bridge.json. `isBridge` is the bridge folder itself.
  ...
  useEffect(() => {
    ...
    return () => {
      if (openRef.current === create) openRef.current = null;
    };
  });
