...
.DESCRIPTION
  Registers a scheduled task "ClaudeBridge" that:
    - starts `npm run start` (or pnpm / bun) in the bridge root when you log on, and
  ...
  and point it at the same `npm run start`; see docs/24-7-setup.md.
...
# Resolve a package manager - the task runs in a non-interactive context where
# PATH may differ, so bake in the absolute path. `run start` keeps package.json
# as the single definition of how the bridge boots; since that now runs on Node,
# npm is the safe default because it ships with Node itself.
$pmPath = $null
foreach ($name in @("npm", "pnpm", "bun")) {
  $cmd = Get-Command $name -ErrorAction SilentlyContinue
  if ($null -eq $cmd) { continue }
  $source = $cmd.Source
  # Get-Command can hand back the PowerShell shim; cmd.exe can't run a .ps1,
  # so prefer the .cmd sitting next to it.
  if ($source -like "*.ps1") {
    $sibling = [System.IO.Path]::ChangeExtension($source, "cmd")
    if (Test-Path $sibling) { $source = $sibling } else { continue }
  }
  $pmPath = $source
  break
}
if ($null -eq $pmPath) {
  Write-Error "No package manager found on PATH (looked for npm, pnpm, bun). Install Node 20+ - it bundles npm - then re-run."
  exit 1
}
...
# fragile and breaks the `>>` redirection when the runner path / log path /
...
Write-Host "  Command     : $pmPath run start"
